...
diamond symbol signals where an example, exercise, remark, definition and such like ends.}
\end{enumerate}
\medskip
...
The more rigorous mathematical definition
is that we call two random variables
$X, Y$ associated if there exist sets $A, B$ such that 
$\mathbb{P}(X \in A, Y \in B) \neq \mathbb{P}(X \in A)\mathbb{P}(Y \in B)$,
where $\mathbb{P}(E)$ denotes the probability of an event $E$.
For the purposes of this class, though, a conceptual understanding
along the lines of the more informal Definition \ref{def:association}
is sufficient.
\parend
\medskip
