...
ggplot(df_diffs, aes(diffs, y = sample)) +
  ...
  ylab("randomisation") +
  xlim(my_lims) +
  geom_vline(xintercept = mean(diffs), linetype = 2) +
...
mean(abs(df_diffs$diffs - mean(df_diffs$diffs)))
@
...
source("functions/randomisation_pvals.R")
p_complete <- numeric(choose(14, 7))
for (i in seq_len(choose(14, 7))) {
  treatment_idx <- combinations[[i]]
  control_idx <- setdiff(1:14, treatment_idx)
  outcome <- c(learners$MethodB[treatment_idx], learners$MethodA[control_idx])
  p_complete[[i]] <- exh_rerand_pval(outcome, 1:7, plot = FALSE)$`right-sided p-value`
  if (i %% 100 == 1) {
    message(paste0("Iteration ", i, " completed."))
  }
}
hist(p_complete)
mean(p_complete <= 0.05) # 61%!

# Using blocking
exh_rerand_pval_blocking <- function(outcome,
                                     treatment_idx,
                                     block,
                                     statistic = mean_diff,
                                     plot = TRUE,
  ...
  block_levels <- unique(block)

  # For each block, generate all admissible treatment allocations
  block_allocs <- lapply(block_levels, function(b) {
    idx <- which(block == b)
    n_treat <- sum(idx %in% treatment_idx)
    combn(idx, n_treat, simplify = FALSE)
  })

  # Cartesian product of the blockwise allocations
  all_allocs <- expand.grid(block_allocs, KEEP.OUT.ATTRS = FALSE)
  treatment_sets <- lapply(seq_len(nrow(all_allocs)), function(i) {
    unlist(all_allocs[i, ], use.names = FALSE)
  })

  H0_stats <- vapply(treatment_sets,
                     function(idx) statistic(outcome, idx),
                     0
  )
  ...
  p_l <- mean(H0_stats <= obs_stat + eps)
  p_r <- mean(H0_stats >= obs_stat - eps)
  p_two <- min(2 * min(p_l, p_r), 1)

  list(
    "left-sided p-value"  = p_l,
    "right-sided p-value" = p_r,
    "two-sided p-value"   = p_two
  )
}
learners$L1 <- factor(learners$L1)
learners$L1 <- as.numeric(learners$L1)
p_blocked <- numeric(choose(14, 7))
for (i in seq_len(choose(14, 7))) {
  if (!retained[[i]]) next
  treatment_idx <- combinations[[i]]
  control_idx <- setdiff(1:14, treatment_idx)
  outcome <- c(learners$MethodB[treatment_idx], learners$MethodA[control_idx])
  block <- c(learners$L1[treatment_idx], learners$L1[control_idx])
  p_blocked[[i]] <- exh_rerand_pval_blocking(outcome, 1:7, block, plot = FALSE)$`right-sided p-value`
  if (i %% 100 == 1) {
    message(paste0("Iteration ", i, " completed."))
  }
}
p_blocked <- p_blocked[p_blocked != 0]
hist(p_blocked)
mean(p_blocked <= 0.05) # only 18%???
@
