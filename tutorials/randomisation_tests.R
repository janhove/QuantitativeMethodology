...
#' # Preliminaries
#' The file `functions/randomisation_pvals.R` defines a few
...
library(here)
source(here("functions", "randomisation_pvals.R"))
...
#' the mean difference is about 0.84.
...
exh_rerand_pval(d$outcome, which(d$group == "treatment"),
                statistic = median_diff, xlab = "Median difference")
#' Inspect the functions `mean_diff()` and `median_diff()`
#' in the R file if you want to define your own test statistics.

exh_rerand_pval(d$outcome, which(d$group == "treatment"),
                statistic = rank_sum, xlab = "Rank sum in treatment condition")

exh_rerand_pval(d$outcome, which(d$group == "treatment"),
                statistic = prob_super, xlab = "Prob. of superiority of treatment condition")
...
mc_pval(d$PropCorrect, which(d$LearningCondition == "ij-ei"),
        ...
        xlab = "Median difference") # using 50,000 rerandomisations

mc_pval(d$PropCorrect, which(d$LearningCondition == "ij-ei"),
        statistic = prob_super, M = 50000,
        xlab = "Prob. of superiority") # using 50,000 rerandomisations
