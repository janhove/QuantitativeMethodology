...
possible \textsc{dag}s for the Andalusian \textsc{clil} study from Example \ref{ex:clil}.
...
\caption{Reciprocal influences can be represented legally in a \textsc{dag} if you break down the temporal structure. The nodes $A_1$, $A_2$, $A_3$ and $A_4$ represent the variable $A$ measured at four points in time. The value of this variable at a given point in time is determined in part by its value at the previous point in time (e.g., $A_3$ is influenced by $A_2$) as well as by the value of another variable at the previous point in time (e.g., $C_2$ influences $A_3$).}
...
\mypar{Example}
  ...
  is 
  $$0.75 \cdot 0.75 + 0.25 \cdot 0.25 = 0.625,$$
  that is, 62.5\%,
...
\begin{framed}
In sum, unbroken chains may both transmit causality and induce associations;
forks induce associations without causality unless measures are taken 
(e.g., controlling for the confounder);
and inverted forks may induce associations without causality if the collider 
(or one of its descendants) is controlled for.
\end{framed}
...
\begin{description}
 ...
 (e.g., teaching, policy)?
 This question is especially important for applied research.
...
\mypar[Ecological validity]{Example}
  ...
  But for language policy, what's probably more relevant is the extent to which
...
\mypar{Exercise}
  ...
  (e.g., \textit{slim}, which means `clever' in Dutch)?
\parend
...
\mypar{Exercise}
  ...
  \begin{enumerate}
    ...
    \item Who were the participants in this study? 
...
study. 
However, each participant is as likely to end up in the intervention
group as any other participant, and the same goes for the control group.
So \emph{on average}---across
...
\begin{enumerate}[(a)]
 ...
 clicks \texttt{Randomize}, 
 inspects the resulting list,
 and decides to assign
 the first 16 to the control group
 and the others to the experimental group.
 
 \item 32 participants sign up for an experiment.
 The researcher enters their names into \url{http://www.random.org/lists/},
 clicks \texttt{Randomize}, 
 and, without first looking at the resulting list, 
 the first 16 to the control group
...
\mypar[Randomisation and estimation]{Remark}\label{remark:estimation}
  ...
  on causal inference; see Definition \vref{def:potentialoutcomes}).
...
\begin{figure}[tpbh]
  ...
  between the intervention and control scores for each of the 11,440
  possible allocations of participants to the two conditions.
  Averaged over all allocations, the mean difference is also one point.
...
\mypar[Potential outcomes]{Definition}\label{def:potentialoutcomes}
...
\begin{proof}
  Assume an experiment with $m$ units of which $n_1$ are randomly assigned to 
  the intervention
  condition and $n_0 = m - n_1$ are assigned to the control condition;
  i.e., there are $\binom{m}{n_1} = \binom{m}{n_0}$ possible allocations.
  The true average intervention effect is defined as the mean of the
  $m$ per-unit differences between the potential outcomes in the intervention 
  and in the control condition.
  Equivalently, we can compute it as the difference between the mean
  of the $m$ potential outcomes in the intervention condition and the mean
  of the $m$ potential outcomes in the control condition.
  
  The estimate we obtain from any particular random allocation is computed
  as the difference between the mean outcome among the $n_1$ units assigned to
  the intervention condition and the mean outcome among the $n_0$ units assigned
  to the control condition.
  Across all possible allocations, 
  every unit is assigned to the intervention condition equally often;
  this works out to $\frac{n_1}{m} \cdot \binom{m}{n_1} = \binom{m-1}{n_1 - 1}$
  times.
  Similarly, every unit is assigned to the control condition equally often;
  this works out to $\frac{n_0}{m} \cdot \binom{m}{n_0} = \binom{m-1}{n_0 - 1}$
  times.
  
  The mean intervention outcome averaged over all ${m} \choose {n_1}$ allocations
  is therefore the sum of $\binom{m-1}{n_1 - 1}$ times each intervention
  potential outcome divided by $n_1 \cdot \binom{m}{n_1}$.
  You can verify that 
  \[
    \left. \binom{m-1}{n_1 - 1} \middle/  \left(n_1 \cdot \binom{m}{n_1}\right)\right.
    = \frac{1}{m}.
  \]
  So the result is the mean potential intervention outcome over all $m$ units.
  Similarly, the mean control outcome averaged over all $\binom{m}{n_0}$ allocations
  is the mean potential control outcome over all $m$ units.
  Hence, averaged over all $\binom{m}{n_1} = \binom{m}{n_0}$ allocations,
  the mean difference between the groups equals the true average intervention effect.
\end{proof}
...
\mypar{Remark}
  ...
  whose potential outcomes in the intervention group are $0, 0, 3$
  ...
  This counterexample demonstrates that the proof of Theorem \ref{th:unbiased} 
  cannot be adapted to medians.
  In the proof, we first rewrote the mean of the intervention effects
  as the difference between two means; this doesn't work for the median.
  We also were able to compute the average mean intervention and control outcomes
  as averages over $n_1 \cdot \binom{m}{n_1}$ and $n_0 \cdot \binom{m}{n_0}$
  potential outcomes, respectively.
  This doesn't work for the average median outcome.
\parend
...
\mypar{Definition}\label{def:variables}
...
Perhaps the researchers or their assistants (unwittingly)
...
\mypar{Example}
  ...
  rather than for strictly syntactic reasons.
...
in a zig-zag pattern (e.g., \textsc{abcdedcba}\dots) 
rather than give their mind to each question,
so that they can tell the researcher they've participated.
The response patterns in Figure \ref{fig:satisficing}
are similarly indicative of satisficing.
...
A pilot study is a small-scale study conducted before the main study.
 ...
 \item detecting any \term{floor} or \term{ceiling effects}.
  ...
  that many participants score near the top.
  The problem with floor and ceiling effects is that participants may differ
  substantially in their ability yet obtain the same score.
\end{enumerate}
...
in a different fashion, presumably starting from assumptions about
...
This table uses the shorthand $Y_i(0)$ to refer 
...
A particularly strong version of the first account is that the experiment's 
...
The putative results are known as potential outcomes; see Definition
\vref{def:potentialoutcomes}.
...
Under the strong $H_0$, we have $Y_i(0) = Y_i(1)$ for $i = 1, \dots, 9$.
...
\section{Testing the null hypothesis by exhaustive rerandomisation}
...
\begin{figure}
\includegraphics[width=0.8\textwidth]{figure/randomisationtest-1}
...
\mypar[Directional and non-directional tests]{Remark}\label{rem:directional}
  ...
  \textminus 0.82 ($p = \frac{123}{126} \approx$ 0.98).
  ...
  The choice of the statistic used to compare the conditions in a
...
\mypar[More than two conditions]{Exercise}
  ...
  \begin{enumerate}[(a)]
    ...
    exhaustive rerandomisation in this setting.
...
\mypar{Theorem}\label{lemma:boundpval}
  Under the strong null hypothesis, 
...
\begin{proof}
  ...
  aren't all equally likely to occur, and the proof of this theorem can be adapted
...
\mypar{Remark}
  Nothing in the proof of Theorem \ref{lemma:boundpval} depends on the choice
...
could plausibly represent the distribution of the 10,000 $p$-values? 
  \item Which of the four histograms would you most hope to find 
...
\mypar[`Flexible' testing]{Example}\label{ex:flexibletesting}
  Theorem \ref{lemma:boundpval} assumes that the analyst decided 
  ...
  with 4.76\% probability.\marginnote{The proportion of $p$-values below 0.05
  cannot easily be gleaned from the data themselves. 
  For each possible randomisation, I had to compute
  the $p$-value that it would have resulted in.}
  ...
  fluency.
  In this case, the probability of obtaining a $p$-value no larger than $\alpha =$ 0.05
...
exhaustive rerandomisation is computationally impossible for larger samples\marginnote{How many ways 
...
how many \emph{years} would it take to run the test when using exhaustive rerandomisation?} 
...
The guarantee encapsulated in Theorem \ref{lemma:boundpval} also holds for
...
Typically,
the guarantee from Theorem \ref{lemma:boundpval} only holds approximately
...
\mypar{Definition}
A study's statistical \term{power} is the probability with which its significance test
...
that the systematic factors cause.
...
This chapter discussed how the strong null hypothesis can
be tested.
It contrasts with the weak null hypothesis, which posits that
the potential outcomes in the conditions are the same on average, rather
than the same for each unit.
See
\href{https://janhove.github.io/posts/2026-07-13-weak-strong-H0/}{\textit{Testing strong and weak null hypotheses in randomised experiments}}.
...
synonymous; see my booklet 
\href{https://janhove.github.io/resources.html#analysing-quantitative-data-an-introduction-for-language-researchers}{\textit{Analysing quantitative data}}
as well as \citet{Winter2019} for introductions
to statistics for linguists that don't emphasise signficance testing.
Nonetheless, quantitative research in the social sciences
has developed something of a significance fetish,
...
we didn't know this and we wanted to estimate this number (i.e., 3.5) by rolling
the dice and jotting down the number of pips showing face-up.

\begin{itemize}
  \item If we do this just once, we'll just obtain each integer between 1 and 6
...
Clearly, the third `design' (rolling the dice 5 times) is preferable to
...
\end{marginfigure}
 ...
 for instance when the matched variable happens to be a collider
...
\mypar{Definition}
  \term{Blocking} is a procedure in which researchers group together units
  that are similar in some respect.
  Then, within each group (block), the units are randomly assigned to
  the experiment's conditions by means of complete randomisation.
\parend
...
presented in the previous chapter (with the mean difference as the test statistic)
...
You can similarly block on other ordered variables.
...
Having taking this class, you've become the go-to expert
  ...
  friend of yours solicits your input on an experiment she wants to run with
...
In a school-based study, examples include children who don't participate
...
The first technique to prevent learning or fatigue effects 
from biasing the results is to vary the order of the conditions
...
When adopting a Latin square design, you first pick at random a Latin square
of the appropriate size in reduced form.
You then randomly assign the participants to its rows.
Finally, you assign the conditions randomly to the symbols used.

For instance, let's say you picked at random the Latin square in 
...
And even a Latin square of size 50 seems impractical.
...
``[W]e shall \dots survey the strengths and weaknesses of a heterogeneous collection of quasi-experimental designs, each deemed worthy of use \emph{where better designs are not feasible}.'' \citep[p.~34; their emphasis in italics, mine in bold-face]{Campbell1963}
\end{quote}

The goal of observational studies (including quasi-experiments)
...
in drawing causal conclusions from observational studies.
...
In observational studies, control variables are often used to
...
\begin{figure}[tpbh]
  ...
  \caption{Hierarchical regression used to control for the age confound in the relationship between shoe size and vocabulary knowledge. The coloured circles in each panel show data belonging to the same three participants. The straight lines are regression lines. A regression line is the straight line that best captures the tendency in the cloud of data points, according to a definition of `best' that we do not cover here.}
...
\citet{Christenfeld2004} make the same points,
and their take-home message is clear:
\begin{quote}
``Statistical adjustment serves a useful
...
many users realize.''
\citep[abstract]{Christenfeld2004}
\end{quote}

Importantly, 
and seemingly contrary to some researchers' beliefs,
...
them won't be biased. 
Further, for variables such as age, the systematic error (roughly
...
As \citet{Huitema2011} puts it,

\begin{quote}
``[F]allibility in a covariate usually implies that there would be 
more adjustment if the variable were measured without error.'' (p.~569)
\end{quote}
...
more than the expected 5 francs per 100-franc bet. Their winnings or losses are a dreadful measure
of their skill level: they all have the same skill level, which corresponds
to a loss of 5 francs per 100-franc bet.
...
lose about 5 francs per 100-franc bet. Some might get lucky or unlucky twice in a
...
If you administer a reading test to a group of learners
 ...
 The reason is that, if you obtained an abysmal score, you're likely
 ...
 were coming down with the flu. A couple of weeks later, you may
 encounter a topic you know a thing or two about or you may
...
or unwittingly---leverage this flexibility to produce a steady stream
...
\begin{enumerate}[(a)]
  \item Open the app at \url{https://janhove.github.io/qrp}
  and carefully read the description.
  
  \item Click `Simulate!', leaving all settings at their default values.
  Describe what the graph (reproduced here as Figure \ref{fig:app} for your convenience) is showing.

\begin{marginfigure}
\centering
\includegraphics[width=\textwidth]{figure/app}
\caption{When you run the app using its default settings, you'll obtain a graph similar to this one. 
Your graph won't be identical as it is based on simulations with random data.}
\figlabel{fig:app}
\end{marginfigure}
  ...
  \begin{enumerate}[i.]
    ...
    \item What will happen when the correlation between the two 
...
it is useful to know a few basic terms and concepts from descriptive statistics.
...
and readers need sophisticated knowledge about statistics and research
design in order to understand them.
But all too often, results sections are more difficult than they need
...
Both questions are uninteresting as they don't tell you whether the treatment actually worked.
  ...
  \item Draw a graph of the predictions.
  ...
  Also see 
  \href{https://janhove.github.io/posts/2025-10-10-mock-outcomes/}{\textit{Clarifying research questions by sketching possible outcomes}}.
...
\begin{enumerate}
  ...
  \item As of the autumn of 2026, the popular data-sharing platform
        \url{https://osf.io/} will not accept new projects.
        The CERN-hosted platform \url{https://zenodo.org/} seems to be
        a promising general-purpose alternative.
        TROLLing (\url{https://site.uit.no/trolling/about/}) is a data 
        repository specifically for language research,
        and SWISSUbase (\url{https://www.swissubase.ch/}) is a Swiss
        alternative for the social sciences and linguistics.
        
  \item For further guidance, 
        see \citet{Klein2018},
        and \citet{Levenstein2018}.
\end{enumerate}
